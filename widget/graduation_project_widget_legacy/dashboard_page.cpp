...
void SetAlertLevelProperty(QLabel* label, const QString& level) {
    if (label == nullptr) {
        return;
    }
    label->setProperty("alertLevel", level);
...
QString MetricState(double value) {
    if (value >= 0.95) {
...
void DashboardPage::BuildUi() {
    ...
    auto* subtitle = new QLabel(
        QStringLiteral("从病人数量、监测记录、报警状态和模型性能四个维度，快速掌握当前系统运行情况。"));
    ...
    recent_records_table_->setAlternatingRowColors(true);
    ...
    recent_alerts_table_->setAlternatingRowColors(true);
...
void DashboardPage::BuildModelMetricsSection(QVBoxLayout* parent_layout) {
    ...
    auto* info_label = new QLabel(QStringLiteral("模型: %1").arg(model_type));
    ...
    auto make_metric = [&](const QString& name, double value) {
        ...
        value_label->setAlignment(Qt::AlignCenter);
        value_label->setProperty("metricState", MetricState(value));
        QFont font = value_label->font();
        font.setPointSize(18);
        font.setBold(true);
        value_label->setFont(font);
        RefreshWidgetStyle(value_label);
    ...
    if (per_class.size() == 5) {
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            if (val >= 0.95) {
                item->setForeground(QColor(110, 231, 183));
            } else if (val < 0.85) {
                item->setForeground(QColor(252, 165, 165));
            } else {
    ...
    if (cm.size() == 5) {
        ...
        for (int r = 0; r < 5; ++r) {
            ...
            for (int c = 0; c < 5 && c < row.size(); ++c) {
                ...
                item->setTextAlignment(Qt::AlignCenter);
                if (r == c) {
                    item->setBackground(QColor(20, 83, 45));
                    item->setForeground(QColor(110, 231, 183));
                    QFont f = item->font();
                    f.setBold(true);
                    item->setFont(f);
...
void DashboardPage::UpdateSummaryCards() {
    if (patient_count_label_ != nullptr) {
        ...
        SetAlertLevelProperty(patient_count_label_, QStringLiteral("normal"));
    }
    if (record_count_label_ != nullptr) {
        record_count_label_->setText(QString::number(record_count_));
        SetAlertLevelProperty(record_count_label_, QStringLiteral("normal"));
    }
    if (alert_count_label_ != nullptr) {
        alert_count_label_->setText(QString::number(alert_count_));
        SetAlertLevelProperty(alert_count_label_, alert_count_ > 0 ? QStringLiteral("warning")
                                                                   : QStringLiteral("normal"));
    }
    if (unconfirmed_count_label_ != nullptr) {
        unconfirmed_count_label_->setText(QString::number(unconfirmed_count_));
        SetAlertLevelProperty(unconfirmed_count_label_, unconfirmed_count_ > 0 ? QStringLiteral("critical")
                                                                                : QStringLiteral("normal"));
    }
}
