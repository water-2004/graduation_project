...
QString NormalizeAlertLevel(const QString& alert_level) {
    const QString normalized = alert_level.trimmed().toLower();
    if (normalized == QStringLiteral("critical") || normalized == QStringLiteral("warning")) {
        return normalized;
    }
...
void AlertDetailDialog::SetAlertData(const AlertInfo& alert_info, const PatientInfo* patient_info) {
    ...
    level_badge_label_->setProperty("alertLevel", NormalizeAlertLevel(alert_info.alert_level));
    RefreshWidgetStyle(level_badge_label_);
...
void AlertDetailDialog::BuildUi() {
    ...
    level_badge_label_->setProperty("badge", true);
    level_badge_label_->setProperty("alertLevel", QStringLiteral("normal"));
