...
void InferPage::CreateDynamicPanels() {
    ...
    pushButtonBack_ = new QPushButton(QStringLiteral("返回病人列表"), group_box_patient);
...
void InferPage::UpdatePatientBanner() {
    if (!has_current_patient_) {
        labelCurrentPatient_->setText(QStringLiteral("当前监测病人：未选择，请先从病人管理页面进入。"));
        return;
    }
...
void InferPage::ApplyAlertStyle(const QString& alert_level) {
    if (alert_level == QStringLiteral("critical")) {
        ui->lineEditAlert->setStyleSheet(
            QStringLiteral("background-color:#fdecec;color:#b42318;font-weight:600;"));
        return;
    }
    if (alert_level == QStringLiteral("warning")) {
        ui->lineEditAlert->setStyleSheet(
            QStringLiteral("background-color:#fff4e5;color:#b54708;font-weight:600;"));
        return;
    }
    ui->lineEditAlert->setStyleSheet(
        QStringLiteral("background-color:#eef8f1;color:#17663a;font-weight:600;"));
}
...
void InferPage::AddHistoryEntry(const QString& pred_label, const QString& confidence,
    ...
    for (int i = 0; i < history_entries_.size(); ++i) {
        ...
        // Color-code alert level cell
        if (e.alert_level == QStringLiteral("critical")) {
            historyTable_->item(i, 3)->setBackground(QColor(0xFD, 0xEC, 0xEC));
            historyTable_->item(i, 3)->setForeground(QColor(0xB4, 0x23, 0x18));
        } else if (e.alert_level == QStringLiteral("warning")) {
            historyTable_->item(i, 3)->setBackground(QColor(0xFF, 0xF4, 0xE5));
            historyTable_->item(i, 3)->setForeground(QColor(0xB5, 0x47, 0x08));
        }
    }
